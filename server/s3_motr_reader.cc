...
bool S3MotrReader::ValidateStoredChksum() {
  ...
  s3_log(S3_LOG_ERROR, "",
         "data_buffer_count(%u), pi_buffer_count(%u), databuf(%p), pibuf(%p)\n",
         data_buffer_count, pi_buffer_count, databuf, pibuf);

  assert(data_buffer_count % pi_buffer_count == 0);

  uint32_t pi_to_data_buffer_ratio = data_buffer_count / pi_buffer_count;

  s3_log(S3_LOG_ERROR, "", "%s pi_to_data_buffer_ratio %u", __func__,
         pi_to_data_buffer_ratio);
  s3_log(S3_LOG_ERROR, stripped_request_id, "%s Motr Data Buffer count %u \n",
         __func__, databuf->ov_vec.v_nr);
  s3_log(S3_LOG_ERROR, stripped_request_id, "%s Attr PI Buffer count %u \n",
  ...
  for (uint32_t i = 0; i < pi_buffer_count; i++) {
    ...
    s3_log(S3_LOG_ERROR, "", "%s start_offset %u", __func__, start_offset);
    s3_log(S3_LOG_ERROR, "", "%s end_offset %u", __func__, end_offset);
    s3_log(S3_LOG_ERROR, "", "%s current_index %lu", __func__, current_index);
